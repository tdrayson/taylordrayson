...
it('resolves a real colour for every accent it names', function () {
    foreach (TypeCatalogue::all() as $key => $meta) {
        expect(array_key_exists($meta->accent, TypeColors::all()))
            ->toBeTrue("The {$key} accent token (--color-{$meta->accent}) is not declared in theme.css.");
    }
});

it('names only icons the frontend registry exports', function () {
    // The exported map alone, not the whole file: icons.js names each glyph twice,
    // so searching the import list above passes even when the export entry is gone
    // and Icon.vue resolves the name to null.
    $exported = Str::after(file_get_contents(base_path('resources/js/icons.js')), 'export const icons = {');

    foreach (TypeCatalogue::all() as $key => $meta) {
        expect($exported)->toMatch(
            '/\b'.preg_quote($meta->icon, '/').'\b/',
            "{$key} asks for {$meta->icon}, which resources/js/icons.js does not export.",
        );
    }
});
...
it('describes every type that can be written by hand', function () {
    // forPicker() and the /drafts headings dereference these without a null check,
    // so a missing row is a 500 on the authoring pages rather than a wrong label.
    foreach (AuthorableTypes::forPicker() as $type) {
        expect(TypeCatalogue::for($type['type']))->not->toBeNull("{$type['type']} is authorable but missing from the catalogue.");
    }
});

it('keeps its three groups from overwriting each other', function () {
    $declared = count(TypeCatalogue::timeline()) + count(TypeCatalogue::links()) + count(TypeCatalogue::authoring());

    expect(TypeCatalogue::all())->toHaveCount($declared, 'Two groups declare the same key, so one silently replaced the other.');
});
