...
it('publishes an event as labelled fields in order', function () {
    ...
    expect(array_map(fn ($f) => $f->key, $export->fields))
        ->toBe(['event', 'venue', 'location', 'organiser', 'ends', 'owner', 'ticket_ref'])
        ...
        ->and($export->field('ends')->display)->toBe('13 Sep 2026 22:00')
        ->and($export->field('owner')->display)->toBe(config('identity.name'))
        ->and($export->field('ticket_ref')->raw)->toBe($event->id);
