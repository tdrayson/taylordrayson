...
it('wires month roll-up stats and per-day calendar data', function () {
    ...
    Food::factory()->create(['calories' => 600, 'occurred_at' => '2026-06-10 13:00:00']);
    ...
    get('/2026/06')->assertInertia(fn ($page) => $page
        ...
        ->where('days.10.types', fn ($types) => ! collect($types)->contains('food'))
...
it('loads the day feed and summary stats from the database', function () {
    ...
    Food::factory()->create(['calories' => 600, 'occurred_at' => '2026-06-21 13:00:00']);

    get('/2026/06/21')->assertInertia(fn ($page) => $page
        ->component('Day')
        ->has('items', 3)
        ->where('items.0.iconKey', fn ($key) => in_array($key, ['activity', 'sleep', 'food'], true))
