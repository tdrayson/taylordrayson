...
it('attaches the bearer key and returns the decoded envelope', function () {
    ...
    $profile = app(Client::class)->profile();
...
it('reads the daily food diary from the singular endpoint with date filters', function () {
    ...
    $diary = app(Client::class)->food(['from' => '2026-06-30', 'to' => '2026-06-30']);
...
it('reads the food library from the plural endpoint', function () {
    ...
    expect(app(Client::class)->foods())->toHaveCount(1);
...
it('passes through date filters on daily logs', function () {
    ...
    app(Client::class)->summaries(['from' => '2026-01-01', 'to' => '2026-06-30']);
...
it('returns null from object endpoints and [] from list endpoints on failure', function () {
    ...
    expect(app(Client::class)->profile())->toBeNull()
        ->and(app(Client::class)->foods())->toBe([]);
});
...
it('returns null when no key is configured', function () {
    ...
    expect(app(Client::class)->get('/v1/me'))->toBeNull();
    Saloon::assertNothingSent();
});

it('converts a firestore timestamp to Carbon', function () {
    expect(Client::toCarbon(['_seconds' => 1569283200, '_nanoseconds' => 0]))
        ->toBeInstanceOf(CarbonImmutable::class)
        ->and(Client::toCarbon(['_seconds' => 1569283200])->toDateString())->toBe('2019-09-24')
        ->and(Client::toCarbon(null))->toBeNull()
        ->and(Client::toCarbon(['nope' => 1]))->toBeNull();
});
