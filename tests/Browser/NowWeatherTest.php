...
it('describes the sky the phone actually reported, rather than falling back to another one', function () {
    ...
    $page->assertScript("document.querySelector('[data-testid=\"weather-headline\"]').textContent.trim()", 'Sun winning, on balance.')
        ->assertScript("document.querySelector('[data-testid=\"weather-temp\"]').textContent.trim()", '24°');
});
...
it('draws the same sky in the top bar as on the Now tile', function () {
    ...
    $page->assertScript(<<<'JS'
        (() => {
            ...
            const tile = paths(document.querySelector('[data-testid="weather-icon"]'));
