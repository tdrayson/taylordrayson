...
/**
 * The `films.meta` column, typed.
 *
 * `$ids` and `$tmdb` are always objects, never null, so a reader can say
 * `$film->meta->tmdb->genres` without a null check on the way.
 */
final readonly class FilmMeta implements Arrayable, Castable, JsonSerializable
{
    /** Keys spelled out below. Everything else survives via `$extra`. */
    private const NAMED = ['year', 'runtime', 'ids', 'tmdb'];
