...
class StatsController extends Controller
{
    ...
    /** Earliest date the "All time" preset reaches back to. */
    private const ALL_TIME_START = '2016-01-01';
    ...
    private function days(Carbon $start, Carbon $end): array
    {
        $rows = Activity::query()
            ...
            ->selectRaw("COALESCE(SUM(CASE WHEN type = 'walk' THEN distance END), 0) AS walk_m")
            ->selectRaw("COALESCE(SUM(CASE WHEN type = 'run' THEN distance END), 0) AS run_m")
            ->selectRaw("COALESCE(SUM(CASE WHEN type IN ('ride', 'e-bike-ride') THEN distance END), 0) AS ride_m")
            ->selectRaw("COALESCE(MAX(CASE WHEN type = 'run' THEN distance END), 0) AS run_max_m")
    ...
    private function records(Carbon $start, Carbon $end): array
    {
        ...
        $longestRide = (int) $between(Activity::query()->whereIn('type', ['ride', 'e-bike-ride']))->max('distance');
    ...
    {
        $mode = (string) $request->query('compare', 'previous-period');
