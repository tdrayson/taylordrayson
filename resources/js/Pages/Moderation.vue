...
<template>
    ...
    <div class="max-w-2xl">
        ...
        <section class="mt-8">
            ...
            <p v-if="! waiting.length" class="mt-3 text-sm text-neutral-500">
                Nothing to read.
            </p>
            ...
            <ul v-else class="mt-3 space-y-4">
                <li v-for="item in waiting" :key="`${item.kind}-${item.id}`" class="rounded-lg border border-neutral-100 p-4">
                    <p class="flex flex-wrap items-baseline gap-x-2 text-sm">
                        ...
                        <span class="text-xs text-neutral-500">{{ item.received }}</span>
                        <span v-if="item.kind === 'mention'" class="text-xs uppercase text-neutral-500">Webmention</span>
                    </p>

                    <p v-if="item.email" class="text-xs text-neutral-500">{{ item.email }}</p>

                    <p v-if="item.body" class="mt-2 whitespace-pre-line text-base text-neutral-900">{{ item.body }}</p>
        ...
        <details v-if="spam.comments.length" class="mt-10">
            <summary class="cursor-pointer text-2xs font-semibold uppercase tracking-wider text-neutral-500">
                Spam ({{ spam.comments.length }})
            </summary>

            <ul class="mt-3 space-y-3">
                <li v-for="item in spam.comments" :key="item.id" class="rounded-lg border border-neutral-50 p-3">
                    <p class="text-sm font-semibold text-neutral-900">{{ item.author }}</p>
                    <p class="mt-1 line-clamp-3 whitespace-pre-line text-xs text-neutral-500">{{ item.body }}</p>
