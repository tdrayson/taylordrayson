                ...
                <section :class="CARD">
                    ...
                    <dl class="flex flex-col gap-4">
                        <div v-for="item in perWeek" :key="item.label" class="flex items-baseline justify-between gap-3">
                            ...
                            <Heading as="dd" size="title" :title="exactDistance(item.distanceM, item.precision)" class="tabular-nums text-neutral-900">{{ item.distanceM != null ? distance(item.distanceM, item.precision) : item.display }}</Heading>
                        </div>
                    </dl>
                </section>
                ...
                <section class="flex flex-1 flex-col" :class="CARD">
                    ...
                    <dl class="flex flex-col gap-4">
                        <div v-for="record in records" :key="record.label" class="flex items-baseline justify-between gap-3">
                            ...
                            <Heading as="dd" size="title" :title="exactDistance(record.distanceM, record.precision)" class="tabular-nums text-neutral-900">{{ record.distanceM != null ? distance(record.distanceM, record.precision) : record.value }}</Heading>
                        </div>
                    </dl>
                </section>
            </div>
