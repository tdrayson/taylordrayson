...
<template>
    ...
    <p class="text-eyebrow uppercase text-ink-3">Error {{ status }}</p>

    <h1 class="mt-3 font-display text-display">{{ copy.title }}</h1>

    <template v-if="status === 404">
        <p class="mt-4 max-w-2xl text-body text-ink-3">
            ...
            <span class="font-semibold text-ink">{{ entries.toLocaleString() }} entries</span>
            over a
            <span class="font-semibold text-ink">{{ days.toLocaleString() }}-day streak</span>, so I genuinely didn't expect anyone to wander this far off the map.
        </p>
        <p class="mt-4 max-w-2xl text-body text-ink-3">
    ...
    <p v-else class="mt-4 max-w-xl text-body text-ink-3">{{ copy.body }}</p>
