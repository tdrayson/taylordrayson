...
const explore = [
    { label: 'Stories', description: 'Editorial long-reads built from the data', href: '/stories', icon: BookOpen01Icon },
    { label: 'Photos', description: 'Every photo on the timeline', href: '/photos', icon: Image01Icon },
    { label: 'Leaderboard', description: 'Snake high scores', href: '/leaderboard', icon: ChampionIcon },
    { label: 'Search', description: 'Find anything, with filters', href: '/search', icon: Search01Icon },
    { label: `${props.currentYear} in days`, description: 'The calendar, one year at a time', href: `/${props.currentYear}`, icon: Calendar03Icon },
];

const site = [
    { label: 'Feeds', description: 'RSS and JSON, filterable by type', href: '/feeds', icon: RssIcon },
    { label: 'Pages', description: 'Standalone pages that live outside the timeline', href: '/pages', icon: File01Icon },
...
<template>
    ...
    <section>
        ...
        <ul class="grid gap-x-8 sm:grid-cols-2">
            <li v-for="item in explore" :key="item.href">
                <Link
                    :href="item.href"
                    class="group flex items-center gap-3 rounded-md py-2.5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
                >
