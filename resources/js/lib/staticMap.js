...
/**
 * Static map for an encoded polyline route (e.g. an activity GPS trace). Uses
 * Mapbox's own auto-fit, since there are no fixed-size markers to keep stable.
 *
 ...
 */
export function staticRouteMap(polyline, { color = '2e9e6a', padding = 64, width = 1280, height = 720 } = {}) {
    if (!polyline) {
        return null;
    }
    ...
    return (
        ...
        `/auto/${width}x${height}@2x?padding=${padding}&attribution=false&logo=false&access_token=${MAPBOX_TOKEN}`
    );
}
...
function worldCircle([cx, cy], radius, segments = 28) {
    ...
    for (let index = 0; index <= segments; index += 1) {
        ...
        const [lng, lat] = worldToLngLat(cx + radius * Math.cos(theta), cy + radius * Math.sin(theta));
        points.push([lat, lng]);
    }
...
function worldDash(a, b) {
    ...
    return encodePolyline([[start[1], start[0]], [end[1], end[0]]]);
}
 ...
 */
export function staticArcMap(origin, destination, { color = '209fdf', padding = 60, width = 1280, height = 720 } = {}) {
    ...
    const world = greatCircle(from, to, 128).map(([lng, lat]) => lngLatToWorld(lng, lat));
    ...
    const [centerLng, centerLat] = worldToLngLat((minX + maxX) / 2, (minY + maxY) / 2);
    ...
    for (let distance = 0; distance < total; distance += DASH_PX + GAP_PX) {
        const dash = worldDash(at(distance), at(Math.min(distance + DASH_PX, total)));
    ...
    const leftPoint = worldToLngLat(mid[0] - dirX * back + perpX * half, mid[1] - dirY * back + perpY * half);
    const rightPoint = worldToLngLat(mid[0] - dirX * back - perpX * half, mid[1] - dirY * back - perpY * half);
    ...
    overlays.push(`path-6+${color}-1+ffffff-1(${encodeURIComponent(worldCircle(world[0], markerRadius))})`);
    overlays.push(`path-6+${color}-1+ffffff-1(${encodeURIComponent(worldCircle(world[world.length - 1], markerRadius))})`);
