...
const props = defineProps({
    ...
    // Standard entry URL (`TvEpisode::url()`), so an episode row goes to its
...
<template>
    ...
    >
        <span class="flex min-w-0 items-baseline gap-3">
            ...
            <span class="truncate text-sm font-medium text-neutral-900 transition-colors group-hover:text-accent-500">{{ title }}</span>
        </span>
        <span class="flex shrink-0 items-center gap-3">
            <span v-if="rating" class="flex items-center gap-1 text-xs text-neutral-500">
                <Icon :icon="StarIcon" class="size-3.5" />
                {{ rating }}
            </span>
            <time v-if="watchTime" :datetime="occurredAt" class="text-xs text-neutral-500 tabular-nums">{{ watchTime }}</time>
        </span>
    </Link>
</template>
