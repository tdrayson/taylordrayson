...
const VARIANTS = {
    info: { wrap: 'border-accent bg-accent-tint', icon: 'text-accent', glyph: InformationCircleIcon },
...
<template>
    <div :class="cn('flex gap-3 rounded-lg border-l-2 px-4 py-3', tone.wrap, props.class)">
        ...
        <div class="min-w-0 text-meta text-ink-2">
            <p v-if="title" class="mb-0.5 font-semibold text-ink">{{ title }}</p>
            <slot />
        </div>
    </div>
</template>
