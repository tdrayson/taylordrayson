...
defineProps({
    ...
    // { day, month, year } parts, pre-formatted server-side so no client date maths.
...
<template>
    ...
    <div class="relative block" :style="{ '--type-color': 'var(--color-flight)' }">
        ...
        <Link :href="href" class="type-color text-label uppercase underline-offset-4 hover:underline focus-visible:underline">Trip</Link>
        ...
        <div class="mt-3 flex max-w-md items-center justify-between gap-4">
            <div class="shrink-0">
                ...
                <div class="font-display text-section">{{ start.month }}</div>
                <div v-if="spansYears" class="text-caption text-neutral-500 tnum">{{ start.year }}</div>
            </div>
            ...
            <div class="shrink-0 text-right">
                ...
                <div class="font-display text-section">{{ end.month }}</div>
                <div v-if="spansYears" class="text-caption text-neutral-500 tnum">{{ end.year }}</div>
            </div>
        </div>
    </div>
</template>
