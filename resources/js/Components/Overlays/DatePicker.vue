        ...
        <div v-if="open" class="absolute inset-x-0 z-50 mt-2 rounded-lg border border-neutral-50 bg-neutral-0 p-3 shadow-card sm:right-auto sm:w-64">
            <template v-if="pickerView === 'days'">
                <div class="flex items-center justify-between">
                    <button type="button" aria-label="Previous month" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepMonth(-1)">
                        <Icon name="ArrowLeft01Icon" class="size-4" />
                    </button>
                    <button type="button" class="rounded px-2 py-0.5 text-sm font-semibold text-neutral-900 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="pickerView = 'months'">
                        {{ monthNames[view.month] }} {{ view.year }}
                    </button>
                    <button type="button" aria-label="Next month" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepMonth(1)">
                        <Icon name="ArrowRight01Icon" class="size-4" />
                    </button>
                </div>
                <div class="mt-3 grid grid-cols-7 gap-1 text-center">
                    <span v-for="(weekday, index) in weekdays" :key="index" class="text-2xs font-semibold text-neutral-500">{{ weekday }}</span>
                    <template v-for="(cell, index) in days" :key="index">
                        <button
                            v-if="cell"
                            type="button"
                            class="rounded border-2 border-transparent py-1 text-xs tabular-nums transition-colors"
            ...
            <template v-else-if="pickerView === 'months'">
                <div class="flex items-center justify-between">
                    <button type="button" aria-label="Previous year" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepYear(-1)">
                        <Icon name="ArrowLeft01Icon" class="size-4" />
                    </button>
                    <button type="button" class="rounded px-2 py-0.5 text-sm font-semibold text-neutral-900 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="pickerView = 'years'">
                        {{ view.year }}
                    </button>
                    <button type="button" aria-label="Next year" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepYear(1)">
                        <Icon name="ArrowRight01Icon" class="size-4" />
                    </button>
                </div>
                <div class="mt-3 grid grid-cols-3 gap-1.5 text-center">
                    <button
                        ...
                        :key="index"
                        type="button"
                        class="rounded border-2 border-transparent py-2 text-xs transition-colors"
            ...
            <template v-else>
                <div class="flex items-center justify-between">
                    <button type="button" aria-label="Previous years" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepDecade(-1)">
                        <Icon name="ArrowLeft01Icon" class="size-4" />
                    </button>
                    <span class="text-sm font-semibold text-neutral-900 tabular-nums">{{ yearRange[0] }} – {{ yearRange[11] }}</span>
                    <button type="button" aria-label="Next years" class="rounded p-1 text-neutral-500 transition-colors hover:text-accent-500 focus-visible:text-accent-500" @click="stepDecade(1)">
                        <Icon name="ArrowRight01Icon" class="size-4" />
                    </button>
                </div>
                <div class="mt-3 grid grid-cols-3 gap-1.5 text-center">
                    <button
                        ...
                        :key="year"
                        type="button"
                        class="rounded border-2 border-transparent py-2 text-xs tabular-nums transition-colors"
