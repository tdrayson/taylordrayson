...
<template>
    <div>
        ...
        <ol v-if="entries.length" :class="showHeading ? 'mt-3' : ''">
            ...
            >
                <span class="tabular-nums w-6 text-meta" :class="isYou(entry) ? 'text-accent-500' : 'text-neutral-500'">{{ index + 1 }}</span>
                ...
                <span class="tabular-nums w-12 text-right text-body font-semibold" :class="isYou(entry) ? 'text-accent-500' : 'text-neutral-900'">{{ entry.score }}</span>
            </li>
        </ol>
