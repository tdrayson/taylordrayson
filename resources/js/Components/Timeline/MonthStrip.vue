    ...
    <nav v-if="months.length" :aria-label="`Months of ${year}`" class="flex flex-wrap gap-x-4 gap-y-2">
        <template v-for="entry in months" :key="entry.month">
            <Link
                ...
                class="text-sm underline-offset-4 transition-colors hover:text-accent-500 hover:underline focus-visible:text-accent-500 focus-visible:underline focus-visible:outline-none"
